          ...
          v-table(v-if="timings.length").overflow-x-auto
            ...
            tbody
              tr(v-for="(timing, index) in timings" :key="timing")
                ...
                td
                  v-radio-group(v-model="timing.mode" density="comfortable" color="primary" hide-details)
                    v-radio(:value="AskMode.FORWARD" label="着手予定の共有")
                    v-radio(:value="AskMode.BACKWARD" label="着手実績の報告")
                td
